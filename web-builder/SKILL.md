...
```
查找：当前工作目录下的 .web_builder/plan.md
```
...
用 [plan-template.md](plan-template.md) 中的模板创建 `.web_builder/plan.md`。
...
**使用 `file(brief=..., action=write, path=..., content=...)` 工具将内容写入文件**：
...
```bash
# 安装依赖
bash "$SKILL_PATH_BASH/web-builder/scripts/web_builder.sh" install /path/to/project

# 构建
bash "$SKILL_PATH_BASH/web-builder/scripts/web_builder.sh" build /path/to/project
```
...
```bash
bash "$SKILL_PATH_BASH/web-builder/scripts/web_builder.sh" start /path/to/project
# 指定端口
bash "$SKILL_PATH_BASH/web-builder/scripts/web_builder.sh" start /path/to/project --port 3000
```

**关键点**：
- 自动分配随机端口（10000-65535），避免冲突
- 输出本地访问和局域网访问地址
...
build 通过、项目验证无误后，将源码打包为 zip 文件供用户下载或归档。

**打包命令**：

```bash
# 打包项目（自动排除 node_modules、.git、.web_builder 等无关目录）
bash "$SKILL_PATH_BASH/web-builder/scripts/web_builder.sh" package /path/to/project -o /path/to/output.zip

# 若需包含 node_modules（通常不需要）
bash "$SKILL_PATH_BASH/web-builder/scripts/web_builder.sh" package /path/to/project -o /path/to/output.zip --include-node-modules
```

**默认排除**：`node_modules`、`.git`、`.web_builder`、`__pycache__`、`.cache`、`.vite`、`.next`、`.nuxt`、`.DS_Store`。

打包完成后告知用户 zip 文件路径和大小。
